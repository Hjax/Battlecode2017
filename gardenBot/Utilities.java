...
public class Utilities extends Bot{
    ...
    {    	
        return tryMove(dir,10,10, rc.getType().strideRadius);
    }
	...
	public static void moveTo(MapLocation destination) throws GameActionException
	{
		if (rc.getLocation().equals(destination) == false)
		{
			...
			else tryMove(rc.getLocation().directionTo(destination),5, 20, distance); 
	...
	public static MapLocation moveBetween(MapLocation source, MapLocation other, float desiredDistance)
	{
		int bytes = Clock.getBytecodeNum();
        // First, try intended direction
        if (rc.canMove(source.add(source.directionTo(other), desiredDistance))) {
            return source.add(source.directionTo(other), desiredDistance);
        }

        // Now try a bunch of similar distances
        boolean moved = false;
        int currentCheck = 1;
        float tryClose = desiredDistance;
        float tryFar = desiredDistance;
        
        while(currentCheck<=10) 
        {
        	// Try the offset of the close side
        	tryClose -= 0.1;
        	if(tryClose > 0 && rc.canMove(source.add(source.directionTo(other), tryClose))) {
        		bytes = Clock.getBytecodeNum() - bytes;
        		System.out.println("moving used " + bytes);
        		return source.add(source.directionTo(other), tryClose);
        	}
        	// Try the offset on the right side
        	tryFar += 0.1;
        	if(rc.canMove(source.add(source.directionTo(other), tryFar))) {
        		bytes = Clock.getBytecodeNum() - bytes;
        		System.out.println("moving used " + bytes);
        		return source.add(source.directionTo(other), tryFar);
        	}
        	// No move performed, try slightly further
        	currentCheck++;
        }
